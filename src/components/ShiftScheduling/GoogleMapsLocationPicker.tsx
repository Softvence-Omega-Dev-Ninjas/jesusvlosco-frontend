...
const GoogleMapsLocationPicker: React.FC<GoogleMapsLocationPickerProps> = ({
  ...
  React.useEffect(() => {
    if (value) {
      setAddressText(value.address || `${value.latitude.toFixed(6)}, ${value.longitude.toFixed(6)}`);
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        const locationData: LocationCoordinates = {
          latitude: lat,
          longitude: lng,
          address: address || `Current Location (${lat.toFixed(6)}, ${lng.toFixed(6)})`
        };
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[95vh] overflow-hidden">
            ...
            <div className="relative">
              ...
              <div className="absolute top-4 left-4 bg-white bg-opacity-90 rounded-md p-3 shadow-md max-w-xs">
                <p className="text-sm text-gray-700">
                  <strong>Click anywhere on the map</strong> to select a location, or use the "Current Location" button to use your GPS location.
                </p>
              </div>
            </div>
              ...
              <div className="p-4 border-t bg-gray-50">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2">{selectedPosition.lat.toFixed(6)}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-700">Longitude:</span>
                    <span className="ml-2">{selectedPosition.lng.toFixed(6)}</span>
                  </div>
                  <div className="md:col-span-1">
                    <span className="font-medium text-gray-700">Address:</span>
                    <span className="ml-2 text-xs">{value?.address || 'Loading...'}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between p-4 border-t">
              <div className="text-sm text-gray-600">
                {selectedPosition ? 'Location selected' : 'Click on the map to select a location'}
              </div>
