...
export const SummaryStep: React.FC<SummaryStepProps> = ({ settings }) => {
  ...
  const handleSurveyPublish = async () => {
    ...
    const savedTitleData = JSON.parse(
      localStorage.getItem("surveyTitle") || "null"
    );
    ...
    const surveyData = {
      ...
      publishTime: settings?.publishNow
        ? new Date().toISOString()
        : settings?.publishTime,
      reminderTime: new Date(
        settings.reminderDate + "T" + settings.reminderTime
      ).toISOString(),
    };
  ...
  const handlePoolPublish = async () => {
    ...
    const poolInfo = {
      ...
      publishTime: settings.publishNow
        ? new Date().toISOString()
        : settings.publishTime,
      reminderTime: settings.sendReminder
        ? new Date(
            `${settings.reminderDate}T${settings.reminderTime}`
          ).toISOString()
        : null,
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-0">
      <div className="bg-gray-100 rounded-lg p-6 sm:p-12 text-center">
        <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 mb-4">
          {flag === "pool" ? "Pool is Live" : "Survey is Live"}
        </h2>
        ...
        <p className="text-gray-600 mb-6">
          Asset will be published on 21/06/2025 at 16:40
        </p>
        <p className="text-gray-600 mb-6">User will be notified :</p>
        <p className="text-gray-600 mb-6 border  border-gray-300 py-2 rounded-xl">
        ...
        <div className="space-y-3 text-sm text-gray-600">
          <p>
            ...
            <span className="font-medium">
              {settings.publishNow
                ? "now"
                : formatDateTime(settings.publishDate, settings.publishTime) ||
                  "21/06/2025 at 16:40"}
            </span>
          </p>
