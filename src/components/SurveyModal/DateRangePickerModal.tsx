...
const monthNameToNumber = (monthName: string): number => monthNames.indexOf(monthName);
...
const DateRangePickerModal: React.FC<DateRangePickerModalProps> = ({ isOpen, onClose }) => {
  ...
  const calendarDays = React.useMemo(() => {
    ...
    if (remainingSlots !== 7) daysArray.push(...Array(remainingSlots).fill(null));
  ...
  const onCalendarDayClick = (day: number) => {
    ...
    if (activeCalendarDateType === 'from') {
      ...
      const from = new Date(
        ...
        parseInt(fromDate.day)
      );
      const to = new Date(
        parseInt(newDate.year),
        monthNameToNumber(newDate.month),
