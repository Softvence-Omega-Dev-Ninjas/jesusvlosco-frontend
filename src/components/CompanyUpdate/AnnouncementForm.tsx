...
const AnnouncementForm = ({ setShowForm }: { setShowForm: React.Dispatch<React.SetStateAction<boolean>> }) => {
  ...
  const { data: teamData } = useGetAllTeamsQuery(null);
  const teams = teamData && teamData.data ? teamData.data.teams : [];
  ...
  return (
    <div className="w-full max-w-8xl mx-auto p-4 sm:p-6 md:p-10 bg-white rounded-md shadow">
      ...
      <input
        ...
        className="w-full mb-4 p-2 border border-gray-300 rounded-md focus:ring focus:ring-indigo-200"
      />
      ...
      <div className="border border-gray-300  rounded-md mb-4">
        ...
        <textarea
          ...
          className="w-full p-2 text-sm rounded-b-md focus:outline-none"
      ...
      <div className="flex flex-wrap gap-3 mt-6 justify-end">
        ...
        <button
          ...
          className="px-4 py-2 text-white text-sm font-medium rounded-md bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
        >
          Publish
        </button>
