...
export default function TimeSheets() {
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Report Date: ${new Date(selectedDate).toLocaleDateString()}`, 25, 55);
    ...
    const filteredEntries = timeSheetEntries.filter(entry =>
      ...
      (entry.shift?.title || '').toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    autoTable(doc, {
      head: [['Employee Name', 'Shift', 'Clock In', 'Clock Out', 'Total Hours', 'Regular Hours', 'Overtime']],
      body: tableData,
      startY: 85,
      theme: 'striped',
      headStyles: {
    ...
    if (finalY < 250) {
      ...
      // Calculate totals
      const totalRegularHours = filteredEntries.reduce((sum, entry) => sum + parseFloat(entry.regularHours || '0'), 0);
      const totalOvertimeHours = filteredEntries.reduce((sum, entry) => sum + parseFloat(entry.overTime || '0'), 0);
      const totalAllHours = filteredEntries.reduce((sum, entry) => sum + parseFloat(entry.totalHours || '0'), 0);
      
      doc.text(`Total Employees Present: ${uniqueUserLocations.length}`, 25, finalY + 35);
      doc.text(`Total Regular Hours: ${totalRegularHours.toFixed(2)}`, 25, finalY + 42);
      doc.text(`Total Overtime Hours: ${totalOvertimeHours.toFixed(2)}`, 120, finalY + 35);
      doc.text(`Total All Hours: ${totalAllHours.toFixed(2)}`, 120, finalY + 42);
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text("CONFIDENTIAL - Daily Clock Sheet", 20, doc.internal.pageSize.height - 10);
      doc.text(`Generated on ${new Date().toLocaleDateString()}`, 105, doc.internal.pageSize.height - 10, { align: "center" });
    }
    
    // Save the PDF
    const fileName = `Daily_Clock_Sheet_${new Date(selectedDate).toLocaleDateString().replace(/\//g, '-')}.pdf`;
    doc.save(fileName);
    
    // Show success message
    Swal.fire({
      title: 'PDF Generated Successfully!',
      text: `Daily clock sheet for ${new Date(selectedDate).toLocaleDateString()} has been downloaded.`,
      icon: 'success',
